...
export class RegisterGuildAllCommand implements CommandRegister {
    ...
    constructor(private TOKEN: string, private CLIENT_ID: string, private GUILD_ID: string) {
        this.rest = new REST({version: "10"}).setToken(this.TOKEN);
    }

    async register(commands: CommandCollection): Promise<void> {
        try {
            await this.rest.put(
                Routes.applicationGuildCommands(this.CLIENT_ID, this.GUILD_ID),
                { body: this.parseCollection(commands) }
            );
        } catch (e) {
